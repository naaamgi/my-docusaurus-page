...
3. **보고서 붙여넣기를 항상 염두에** — PoC 섹션은 그대로 복사해서 결함보고서에 들어갈 수 있어야 함.
...
(왜 이 점검을 하는지, 어떤 위험을 확인하려는지 2~3 문장)
...
1. `<TARGET>` 접속 후 로그인
2. `<URL>` 페이지의 `<PARAM>` 파라미터에 아래 페이로드 입력
3. 응답 확인
...
**확인 사항:** (응답에서 무엇이 보이면 취약인지)

## 영향도 분석

- **기밀성 (Confidentiality)**: 영향 정도
- **무결성 (Integrity)**: 영향 정도
- **가용성 (Availability)**: 영향 정도
...
→ 명령어만 나열. 언제 쓰는지, 어떻게 판정하는지, 보고서엔 어떻게 쓰는지 알 수 없음.
